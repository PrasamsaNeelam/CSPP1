...
def paying_debt(balance_up, annualinterest_rate, guess_num):
    '''function for paying debt'''
    balance_copy = balance_up

    iterator_i = 1
    while iterator_i <= 12:
        montly_interest_rate = annualinterest_rate / 12
...
def paying_debt_off_in_a_year(balance_up, annualinterest_rate):
    '''function for paying deft off in a year'''
    approximation_val = 0.03
    montly_interest_rate = annualinterest_rate /12.0
    high_val = (balance_up * (1 + montly_interest_rate) ** 12) / 12.0
    low_val = balance_up / 12
    middle_val = (low_val + high_val) / 2.0

    while abs(paying_debt(balance_up, annualinterest_rate, middle_val)) >= approximation_val:
        if approximation_val <= paying_debt(balance_up, annualinterest_rate, middle_val):
            low_val = middle_val
        else:
            high_val = middle_val
        middle_val = (low_val + high_val) / 2.0
